...
import geoipValidate from "../../validations/geoip.validation";
...
export default async function IPsUpdate() {
  try {
    ...
    if (!nError) {
      ...
      const { geoipError } = geoipValidate(IpsGeoInfo);
      if (!geoipError) {
        for (let i = 0; i < SNByIps[0].length; i++) {
          ...
          if (!IPinDB) {
            ...
            newIP.accuracyRadius =
              IpsGeoInfo[i]?.location?.accuracy_radius || null;
        ...
        throw new ApiError(
          httpStatus.INTERNAL_SERVER_ERROR,
          geoipError.details[0].message
        );
      }
    } else {
      throw new ApiError(
        httpStatus.INTERNAL_SERVER_ERROR,
        nError.details[0].message
      );
    }
  } catch (err) {
    console.log(err);
